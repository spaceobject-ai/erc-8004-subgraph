...
- Entity types that are mutable: `IdentityRegistry`, `ReputationRegistry`,
  ...
  list.
- `Agent` and `Feedback` IDs are UTF-8 strings that start with the chain ID
  (`"<chainId>:<registry>:<agentId>"`), because the registries are deployed at
  the same addresses on several chains. `AgentRegistration` and
  `FeedbackDocument` IDs are `keccak256` of the parent entity ID plus the
  source URI, so each URI revision keeps its own immutable document.
